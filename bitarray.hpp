...
class bitarray {
 public:
  ...
  std::size_t count(std::size_t first, std::size_t last) const {
    std::size_t sum = 0;
    if (lower_index(first) != upper_index(first)) {
      sum += popcount(data[lower_index(first)]
          & (~UINT64_C(0) << (first & 0x3F)));
    } if (lower_index(last) != upper_index(last)) {
      sum += popcount(data[lower_index(last)]
          & (~UINT64_C(0) >> (64-(last & 0x3F))));
    }
